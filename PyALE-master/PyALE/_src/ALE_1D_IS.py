...
def aleplot_1D_categorical2(X, model, feature, encode_fun, predictors, include_CI=True, C=0.95, verbose=False):
    """Compute the accumulated local effect of a categorical (or str) feature.
    ...
    logger.info(F"Starting of {aleplot_1D_categorical2.__qualname__}")
    logger.warning(F"Start: Data types check of the function")
    if type(X)!=type(pd.DataFrame()):
        raise TypeError(f"type of X should be a {type(pd.DataFrame())}")
    ...
    # their similarity to each other, based on the distributions of the other
    # features by group
    logger.debug(F"If condition:if (X[feature].dtype.name != 'category') or (not X[feature].cat.ordered):{(X[feature].dtype.name != 'category') or (not X[feature].cat.ordered)}")
    if (X[feature].dtype.name != "category") or (not X[feature].cat.ordered):
        logger.debug(F"If condition:if (X[feature].dtype.name != 'category'):{(X[feature].dtype.name != 'category')}")
        # logger.debug(F"If condition:if (not X[feature].cat.ordered):{(not X[feature].cat.ordered)}")
        logger.debug(F"If condition:if (X[feature].dtype.name != 'category') or (not X[feature].cat.ordered):{(X[feature].dtype.name != 'category') or (not X[feature].cat.ordered)}")
        X[feature] = X[feature].astype(str)
        groups_order = order_groups(X, feature)
        groups = groups_order.index.values
        logger.debug(F"type(groups_order.index.values):{groups_order.index.values}, groups_order.index.values:{groups_order.index.values}")
        X[feature] = X[feature].astype(
            pd.api.types.CategoricalDtype(categories=groups, ordered=True)
        )
    ...
    log_disable()
    logger.info(F"Ending of {aleplot_1D_categorical2.__qualname__}")
    return res_df
